...
- CORS 白名单 + `credentials`;`/api/auth/*` 基础限流(每 IP 每分钟 20 次)。**本机反代部署须设 `.env` 的 `TRUST_PROXY=loopback`**,仅信任环回代理链中的 `X-Forwarded-For`;否则 `req.ip` 恒为反代地址,全站用户共用一个配额而互相误伤。直连公网保持 `0`,以免伪造该头绕过限流。
