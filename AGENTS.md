...
参考项目/             原始硬编码站点(只读基线)
.agents/skills/     Codex Skill:pigeon-course-authoring(课程包制作,渐进式披露 + 示例)
...
- **AI 执行约定**:编程实现默认交 **codex** 子代理执行,Codex 负责设计与复核(详见 [`docs/agent-workflow.md`](./docs/agent-workflow.md) §一)。**例外**:含中文的既有源文件改写本机 codex 会乱码,改由 Codex 亲自编辑(见不变量 8)。
...
6. **安全**:Codex 不替用户输入任何真实账号/密码/密钥;`server/.env`、密钥、首个管理员口令均由用户本机配置。
7. **文档随代码同步更新**:结构/功能变更时,同步更新对应目录 README、根 README 与本索引(见 agent-workflow)。
8. **中文文件编码**:本机 codex 改写既有中文文件会乱码(见用户记忆);**含中文的既有源文件由 Codex 亲自用 Edit/Write 编辑**,codex 仅用于新建文件或纯逻辑文件,改后须验编码(无 BOM、无 mojibake)。
...
| 文件 | 职责 |
|------|------|
| `AGENTS.md`(本文件) | 项目总纲、目录导览、不变量、设计硬规则、规范索引 |
...
| [`docs/agent-workflow.md`](./docs/agent-workflow.md) | **agent/codex 执行规范**:**编程实现默认交 codex 执行、Codex 设计/复核**;构建/提交/QA、文档维护规则 |
...
| [`.agents/skills/pigeon-course-authoring/`](./.agents/skills/pigeon-course-authoring/SKILL.md) | 课程包制作 Skill(Codex):工作流 + 格式速查 + 打包用法 + 示例课程 |
